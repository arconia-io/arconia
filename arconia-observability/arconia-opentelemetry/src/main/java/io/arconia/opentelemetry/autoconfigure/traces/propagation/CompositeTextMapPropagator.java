 ...
 * <p>
 * Code adapted from <a href="https://github.com/spring-projects/spring-boot/blob/v4.0.1/module/spring-boot-micrometer-tracing-opentelemetry/src/main/java/org/springframework/boot/micrometer/tracing/opentelemetry/autoconfigure/CompositeTextMapPropagator.java">CompositeTextMapPropagator.java</a>
 * authored by Moritz Halbritter and Scott Frederick in the Spring Boot project.
 */
class CompositeTextMapPropagator implements TextMapPropagator {
	 ...
	 */
	CompositeTextMapPropagator(Collection<TextMapPropagator> injectors, Collection<TextMapPropagator> mutuallyExclusiveExtractors, @Nullable TextMapPropagator baggagePropagator) {
	...
	public <C> void inject(Context context, @Nullable C carrier, TextMapSetter<C> setter) {
		if (context != null && setter != null) {
			injectors.forEach((injector) -> injector.inject(context, carrier, setter));
		}
	}
	...
	public <C> Context extract(Context context, @Nullable C carrier, TextMapGetter<C> getter) {
		...
		if (baggagePropagator != null) {
			result = baggagePropagator.extract(result, carrier, getter);
		}
		return result;
	}
	 ...
	 * Creates a new {@link CompositeTextMapPropagator}.
	 */
	static TextMapPropagator create(TracingProperties.Propagation properties, @Nullable TextMapPropagator baggagePropagator) {
	...
	private static List<TracingProperties.Propagation.PropagationType> getEffectiveConsumedTypes(TracingProperties.Propagation properties) {
		return (properties.getType() != null) ? properties.getType() : properties.getConsume();
	}

	private static List<TracingProperties.Propagation.PropagationType> getEffectiveProducedTypes(TracingProperties.Propagation properties) {
		return (properties.getType() != null) ? properties.getType() : properties.getProduce();
	}

	/**
	 * Mapper used to create a {@link TextMapPropagator} from a {@link TracingProperties.Propagation.PropagationType}.
	 */
