...
/**
 * Integration tests for {@link OpenLitOtlpTracingContainerConnectionDetailsFactory}.
 */
@SpringJUnitConfig
@TestPropertySource(properties = "arconia.otel.traces.exporter.type=none")
class OpenLitOtlpTracingContainerConnectionDetailsFactoryTests extends OpenLitTestcontainers {
    ...
    void shouldProvideConnectionDetailsForHttpProtobuf() {
        String url = connectionDetails.getTracesUrl(Protocol.HTTP_PROTOBUF);
        String expectedUrl = "http://localhost:" + openLitContainer.getOtlpHttpPort() + OtlpTracingConnectionDetails.TRACES_PATH;
        assertThat(url).isEqualTo(expectedUrl);
    }

    @Test
    void shouldProvideConnectionDetailsForGrpc() {
        String url = connectionDetails.getTracesUrl(Protocol.GRPC);
