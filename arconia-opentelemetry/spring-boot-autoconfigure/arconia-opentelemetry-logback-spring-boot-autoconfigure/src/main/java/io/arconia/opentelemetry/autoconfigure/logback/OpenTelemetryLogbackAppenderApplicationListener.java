 ...
 */
public class OpenTelemetryLogbackAppenderApplicationListener implements GenericApplicationListener {
    ...
    private void configureOpenTelemetryAppender(OpenTelemetryAppender openTelemetryAppender, Binder binder) {
        boolean captureArguments = binder
                .bind(OpenTelemetryLogbackAppenderProperties.CONFIG_PREFIX + ".capture-arguments",
                        Boolean.class)
                .orElse(false);
        openTelemetryAppender.setCaptureArguments(captureArguments);

        boolean captureCodeAttributes = binder
                .bind(OpenTelemetryLogbackAppenderProperties.CONFIG_PREFIX + ".capture-code-attributes",
                        Boolean.class)
                .orElse(false);
        openTelemetryAppender.setCaptureCodeAttributes(captureCodeAttributes);

        boolean captureExperimentalAttributes = binder
                .bind(OpenTelemetryLogbackAppenderProperties.CONFIG_PREFIX + ".capture-experimental-attributes",
                        Boolean.class)
                .orElse(false);
    ...
    private boolean isLogbackInstrumentationEnabled(Binder binder) {
        return binder.bind(OpenTelemetryLogbackAppenderProperties.CONFIG_PREFIX + ".enabled", Boolean.class)
                .orElse(true);
    }
