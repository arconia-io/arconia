...
/**
 * Unit tests for {@link OpenTelemetryToolCallingObservationConvention}.
 */
class OpenTelemetryToolCallingObservationConventionTests {

    private final OpenTelemetryAiConventionsProperties properties = new OpenTelemetryAiConventionsProperties();
    private final OpenTelemetryToolCallingObservationConvention convention =
            new OpenTelemetryToolCallingObservationConvention(properties);
    ...
    void shouldIncludeContentWhenEnabled() {
        ...
        KeyValues keyValues = convention.getHighCardinalityKeyValues(context);
        assertThat(keyValues).contains(
                KeyValue.of(GenAiAttributes.GEN_AI_TOOL_CALL_ARGUMENTS.getKey(), """
                        {"location": "Paris}"""),
