...
/**
 * Unit tests for {@link OpenLitEmbeddingModelObservationConvention}.
 */
class OpenLitEmbeddingModelObservationConventionTests {

    private final OpenLitEmbeddingModelObservationConvention convention =
            new OpenLitEmbeddingModelObservationConvention();

    @Test
    void shouldHaveName() {
        assertThat(convention.getName())
                .isEqualTo(OpenLitEmbeddingModelObservationConvention.DEFAULT_NAME);
    }
    ...
    @Test
    void shouldUseEmbeddingDimensionKeyWhenDimensionsSet() {
        ...
        assertThat(keyValues).contains(
                KeyValue.of(OpenLitAttributes.GEN_AI_REQUEST_EMBEDDING_DIMENSION, "1536")
        );
        assertThat(keyValues).noneSatisfy(kv ->
                assertThat(kv.getKey()).isEqualTo(
                        GenAiIncubatingAttributes.GEN_AI_EMBEDDINGS_DIMENSION_COUNT.getKey()));
    }

    @Test
    void shouldNotIncludeDimensionKeyWhenDimensionsNotSet() {
        EmbeddingModelObservationContext context = createContext("text-embedding-3-small");

        KeyValues keyValues = convention.getHighCardinalityKeyValues(context);
        assertThat(keyValues).noneSatisfy(kv ->
                assertThat(kv.getKey()).isEqualTo(OpenLitAttributes.GEN_AI_REQUEST_EMBEDDING_DIMENSION));
    }
