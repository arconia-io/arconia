...
/**
 * Unit tests for {@link OpenLLMetryAutoConfiguration}.
 */
class OpenLLMetryAutoConfigurationTests {

    private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
            .withConfiguration(AutoConfigurations.of(OpenLLMetryAutoConfiguration.class));
    ...
    void autoActivatesWhenConventionTypePropertyNotSet() {
        contextRunner.run(context -> {
            ...
            assertThat(context.getBean(ObservationConventionsProvider.class).name()).isEqualTo("openllmetry");
    ...
    @Test
    void activatesWhenConventionTypeExplicitlySetToOpenLLMetry() {
        contextRunner
            .withPropertyValues("arconia.observations.conventions.type=openllmetry")
            .run(context -> {
                assertThat(context).hasSingleBean(ObservationConventionsProvider.class);
                assertThat(context).hasSingleBean(OpenLLMetryChatModelObservationConvention.class);
                assertThat(context).hasSingleBean(OpenLLMetryEmbeddingModelObservationConvention.class);
    ...
    void observationConventionsAvailableWithExplicitConfiguration() {
        contextRunner.withPropertyValues("arconia.observations.conventions.type=openllmetry").run(context -> {
            assertThat(context).hasSingleBean(ObservationConventionsProvider.class);
            assertThat(context).hasSingleBean(OpenLLMetryChatModelObservationConvention.class);
            assertThat(context).hasSingleBean(OpenLLMetryEmbeddingModelObservationConvention.class);
            assertThat(context).hasSingleBean(OpenLLMetryToolCallingObservationConvention.class);
