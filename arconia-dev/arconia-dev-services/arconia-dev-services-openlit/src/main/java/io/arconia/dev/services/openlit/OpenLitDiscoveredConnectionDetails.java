...
final class OpenLitDiscoveredConnectionDetails
    ...
    OpenLitDiscoveredConnectionDetails(DiscoveredContainer container) {
        this.httpUrl = "http://%s:%d".formatted(container.host(), container.mappedPort(OpenLitContainer.OTLP_HTTP_PORT));
        this.grpcUrl = "http://%s:%d".formatted(container.host(), container.mappedPort(OpenLitContainer.OTLP_GRPC_PORT));
    }
