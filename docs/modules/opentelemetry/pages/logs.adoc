...
|`arconia.otel.logs.exporter.otlp.retry.max-attempts`
|`5`
|Maximum number of retries.

|`arconia.otel.logs.exporter.otlp.retry.backoff-config.first-backoff`
|`1s`
|Initial backoff time.

|`arconia.otel.logs.exporter.otlp.retry.backoff-config.max-backoff`
|`5s`
|Maximum backoff time.

|`arconia.otel.logs.exporter.otlp.retry.backoff-config.multiplier`
|`1.5`
|Backoff multiplier.
|===
...
Java applications typically configure logging via SLF4J and use one of the popular implementations such as Logback or Log4J2. Arconia supports bridges to export Logback logs to OpenTelemetry. Support for Log4J2 is planned for a future release.
...
WARNING: The Logback OpenTelemetry Bridge from the OpenTelemetry Java Instrumentation project is still experimental.
...
|`arconia.otel.logs.logback-bridge.capture-template`
|`false`
...
|`arconia.otel.logs.logback-bridge.capture-logstash-marker-attributes`
|`false`
