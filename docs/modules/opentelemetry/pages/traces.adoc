...
The bridge logic is provided by the https://docs.micrometer.io/tracing/reference/tracers.html[Micrometer Tracing] project, and it's bundled with the Arconia OpenTelemetry Spring Boot Starter. Disabling the bridge is not a supported configuration option, since Micrometer Tracing is a core part of the OpenTelemetry support in Spring Boot.
