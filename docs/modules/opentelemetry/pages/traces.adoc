...
|`arconia.otel.traces.exporter.otlp.retry.max-attempts`
|`5`
|Maximum number of retries.

|`arconia.otel.traces.exporter.otlp.retry.backoff-config.first-backoff`
|`1s`
|Initial backoff time.

|`arconia.otel.traces.exporter.otlp.retry.backoff-config.max-backoff`
|`5s`
|Maximum backoff time.

|`arconia.otel.traces.exporter.otlp.retry.backoff-config.multiplier`
|`1.5`
|Backoff multiplier.
|===
...
Spring libraries and many other libraries from the Java ecosystem are instrumented using Micrometer Tracing (via the Micrometer `Observation` API) and rely on Micrometer for context propagation and baggage management. Arconia supports the https://docs.micrometer.io/tracing/reference/tracers.html[Micrometer Tracing OpenTelemetry Bridge] from the Micrometer Tracing project to export such traces via OpenTelemetry.

=== Micrometer Tracing Bridge (from Micrometer)

The Arconia OpenTelemetry Spring Boot Starter comes built-in with the https://docs.micrometer.io/tracing/reference/tracers.html[Micrometer Tracing OpenTelemetry Bridge] provided by the https://docs.micrometer.io/tracing/reference/[Micrometer Tracing] project, which transparently converts Micrometer traces into OpenTelemetry Traces going through the OpenTelemetry API.

Disabling the bridge is not a supported configuration option, since Micrometer Tracing is a core part of the OpenTelemetry tracing support in Spring Boot.
