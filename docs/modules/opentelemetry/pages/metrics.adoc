...
=== Micrometer Metrics Registry OTLP (from Micrometer)
...
NOTE: Since the Micrometer Metrics Registry OTLP exports metrics directly to an OTLP endpoint, the metrics won't be available via the OpenTelemetry API. One of the consequences is that they won't be available when configuring OpenTelemetry with different exporters, such as the console exporter.
...
[source,yaml]
----
arconia.otel.metrics.micrometer-otlp.enabled: false
----
...
The Micrometer Metrics Registry OTLP can be configured via configuration properties.

.Micrometer Metrics Registry OTLP Configuration Properties
|===
|Property |Default |Description
|	`arconia.otel.metrics.micrometer-otlp.base-time-unit`
|	`seconds`
|	Base time unit for Micrometer metrics.

|	`arconia.otel.metrics.micrometer-otlp.max-bucket-count`
|	`160`
...
|	`arconia.otel.metrics.micrometer-otlp.max-scale`
|	`20`
|	Max scale to use for exponential histograms, if configured.
|===

All the configuration properties described earlier for OpenTelemetry Metrics and OTLP exporter also apply to the Micrometer Metrics Registry OTLP.

=== Micrometer Metrics OpenTelemetry Bridge (from OpenTelemetry Java Instrumentation)

Arconia also supports exporting Micrometer metrics via the Micrometer Metrics OpenTelemetry Bridge provided by the https://github.com/open-telemetry/opentelemetry-java-instrumentation/tree/main/instrumentation/micrometer/micrometer-1.5/library[OpenTelemetry Java Instrumentation for Micrometer], which is based on the OpenTelemetry API and integrates fully with the OpenTelemetry SDK.
...
NOTE: The Micrometer Metrics OpenTelemetry Bridge is mutually exclusive with the Micrometer Metrics Registry OTLP. Having both of them in the classpath is not supported and will lead to undefined behavior.
