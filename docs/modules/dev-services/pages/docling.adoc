...
* xref:generative-ai:docling-document-reader.adoc[Docling Document Reader].
...
|`true`
|Whether to enable the Docling UI. When enabled, sets the `DOCLING_SERVE_ENABLE_UI` environment variable to `1`. This is only considered when running the application in dev mode.

|`arconia.dev.services.docling.api-key`
|``
