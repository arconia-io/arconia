...
A service providing a https://github.com/docling-project/docling-serve[Docling Serve] instance for development and testing purposes.

It works with Spring Boot libraries that support Docling, including:

* xref:integrations:docling.adoc[Arconia Docling]
* xref:generative-ai:docling-document-reader.adoc[Arconia AI Docling Document Reader].
...
By default, when running the application in dev mode, the Dev Service is shared across multiple applications running simultaneously: the application connects to a running Docling Dev Service started by another application if available, or else starts its own that other applications can discover in turn. The applications sharing the Dev Service must use the same API key, if any.

Furthermore, the Dev Service is configured to enable the Docling UI. The application logs will show you the URL where you can access that.

[source,logs]
----
...i.a.d.s.startup  : Dev Service 'docling' is ready - Docling UI: http://localhost:<port>/ui, Docling OpenAPI: http://localhost:<port>/docs
----
...
| `0`
| Fixed port for exposing the Docling Serve HTTP port to the host. When it's 0 (default), a random available port is assigned dynamically.
...
| `false`
| Whether the container joins the shared dev services network, so it can communicate with other dev service containers, for example to send telemetry to a Grafana LGTM Dev Service (see xref:dev-services:index.adoc#_shared_network[Shared Network]). Only applicable in dev and test mode.
...
| `arconia.dev.services.{service-name}.enable-ui`
| `true`
| Whether to enable the Docling UI when in dev mode.

| `arconia.dev.services.{service-name}.api-key`
|
