...
To go beyond the built-in default configuration, for example to forward telemetry to another backend, you can supply your own collector configuration file and copy it into the container via the `resources` property (see xref:index.adoc#_resource_mappings[Resource Mappings]). Each mapping requires two values:
