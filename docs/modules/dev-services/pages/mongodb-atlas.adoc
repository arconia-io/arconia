...
* https://spring.io/projects/spring-ai[Spring AI] (Vector Store)
...
[NOTE]
====
This Dev Service is mutually exclusive with other MongoDB dev services. If more than one is active at the same time, the application will fail at startup with a clear error message. Disable all but one by setting `arconia.dev.services.<name>.enabled=false`.
====
