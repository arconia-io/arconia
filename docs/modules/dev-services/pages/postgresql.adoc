...
[WARNING]
====
Some platforms like OpenShift might prevent PostgreSQL from writing data due to how permissions are configured for the container runtime. A possible workaround is configuring PostgreSQL to store data in a location inside the container where the platform allows writing: `arconia.dev.services.postgresql.environment.PGDATA=/tmp/pgdata`.
====
