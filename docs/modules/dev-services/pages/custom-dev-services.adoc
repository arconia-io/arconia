...
* `arconia-dev-services-api`: lightweight contracts with no Docker/Testcontainers dependencies, such as the configuration property interfaces (`BaseDevServicesProperties`, `JdbcDevServicesProperties`, `SharedDevServicesProperties`), the runtime descriptors (`DevServiceRegistration`, `ContainerInfo`), and the container labels Dev Services are identified and described by (`DevServiceLabels`). It's designed to be safely usable in contexts where Testcontainers should not be on the classpath, such as tooling or user interfaces built around Dev Services.
...
public final class FancyDbContainer extends GenericContainer<FancyDbContainer> {
    ...
    public FancyDbContainer(FancyDbDevServicesProperties properties) {
        ...
        this.withExposedPorts(FANCYDB_PORT);
...
Unless your container class already declares a wait strategy of its own, declare one explicitly before calling `ContainerConfigurer.base()`, as in the example above. Testcontainers shares a single default wait strategy instance across all containers that don't declare one, and `ContainerConfigurer.base()` applies the `startup-timeout` property to whichever wait strategy the container has at that point. Without an explicit wait strategy, the startup timeout you configure for your Dev Service is applied to the shared instance, changing the startup timeout of every other container relying on it as well. Container classes provided by Testcontainers vary in this respect: `GenericContainer`, `MySQLContainer`, and `OllamaContainer` rely on the shared instance, whereas `PostgreSQLContainer` and `RabbitMQContainer` declare their own.
...
If your service has endpoints worth surfacing to developers, such as a management console or a telemetry endpoint, implement `DevServiceLinkProvider` on your container class and declare one `DevServiceLinkDefinition` per link. Declare the container port rather than a URL: Arconia resolves the URL once the port mapping is known.
...
The links appear in the startup message and in the `devservices` actuator endpoint (see xref:index.adoc#_startup_feedback[Startup Feedback]). Arconia also records each definition on the container as one label per field, so an application that adopts your container as a shared Dev Service reports the same links without declaring them again:

[source,text]
----
io.arconia.dev-service.link.fancydb.label  = FancyDB Console
io.arconia.dev-service.link.fancydb.scheme = http
io.arconia.dev-service.link.fancydb.port   = 8090
io.arconia.dev-service.link.fancydb.path   = /console
----

The port is the container port, since no port is mapped when the labels are applied. Tools outside the JVM can read a single value with `docker inspect --format '{{index .Config.Labels "io.arconia.dev-service.link.fancydb.port"}}'`. Because the labels are written by the application that started the container, the links describe the service as that application configured it, including any link it chose not to expose.

Keep these points in mind when declaring links:

* `devServiceLinkDefinitions()` is called before the container starts, so it must not read anything only a running container has, such as a mapped port. Capture whatever the links depend on when the container is constructed.
* The link id becomes part of the label name, so it's restricted to lowercase letters, digits, and dashes, and each link needs a distinct one.
* Declare a port your container actually exposes. A link whose port can't be resolved is skipped with a warning, and the remaining links are still reported.
* Like all container labels, these take part in the Testcontainers reuse hash. They're stable across runs, so reuse keeps working, but changing a link means the next run starts a new container instead of reusing the previous one.
...
Declaring the discovery specification is what opts your Dev Service into discovery: a service that declares none never participates, even when the `shared` property is enabled. Container reuse is a separate, orthogonal concern, applied from the `reuse` property by `ContainerConfigurer` when the container is created (via `ContainerConfigurer.base(...)`), so a reused container can also be shared.
...
The shared network (see xref:index.adoc#_shared_network[Shared Network]) is a global feature: when the user sets `arconia.dev.services.network.enabled`, every Dev Service container joins the network automatically. Your Dev Service needs no special declaration to participate.
