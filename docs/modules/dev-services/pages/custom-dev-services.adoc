...
To let your Dev Service container communicate with other Dev Service containers over an OCI network (see xref:index.adoc#_shared_network[Shared Network]), have your properties class implement `NetworkedDevServicesProperties`, which adds the `join-network` property to its contract, and declare a network specification when registering the service:
...
`join-network` is independent of `shared`: it can be combined with a sharing specification (a properties class may implement both `SharedDevServicesProperties` and `NetworkedDevServicesProperties`), or used on its own. Applies in both dev and test mode.

A networked container is reachable by its service name by default, so it always has a stable name other containers can use. Users can override that with the common `network-aliases` property, so make sure your container applies it: using `ContainerConfigurer.base(...)`, which calls `withNetworkAliases(...)`, is enough.
