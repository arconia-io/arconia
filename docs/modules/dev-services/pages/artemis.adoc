...
A service providing an https://activemq.apache.org/components/artemis/[ActiveMQ Artemis] event broker for development and testing purposes.

It works with Spring Boot libraries that support JMS and ActiveMQ Artemis, including:
...
By default, when running the application in dev mode, the Dev Service is shared across multiple applications running simultaneously: the application connects to a running Artemis Dev Service started by another application if available, or else starts its own that other applications can discover in turn. The applications sharing the Dev Service must use the same credentials.

== Accessing the Management Console

The application logs will show you the URL where you can access the ActiveMQ Artemis Management Console.

[source,logs]
----
...i.a.d.s.startup  : Dev Service 'artemis' is ready - Artemis Management Console: http://localhost:<port>/console
----
