...
It works with Spring Boot libraries that support RESP or Redis, including:
...
[NOTE]
====
This Dev Service is mutually exclusive with other Redis dev services. If more than one is active at the same time, the application will fail at startup with a clear error message. Disable all but one by setting `arconia.dev.services.<name>.enabled=false`.
====
...
| `0`
| Fixed port for exposing the Redis database port to the host. When it's 0 (default), a random available port is assigned dynamically.
