...
Built on https://testcontainers.com[Testcontainers] and https://docs.spring.io/spring-boot/reference/features/dev-services.html#features.dev-services.testcontainers[Spring Boot's Testcontainers support], the Arconia Dev Services are designed to provide a great developer experience by eliminating the friction of setting up and managing those services manually in your development environment.
...
You can globally disable all Dev Services by setting the `arconia.dev.services.enabled` configuration property.
...
| Service Sharing
| Built-in sharing modes via configuration properties.
