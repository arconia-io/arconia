...
class GreetingController {
    ...
    @GetMapping("/greeting")
    Greeting greeting(@TenantIdentifier String tenantId) {
        return new Greeting("Hello", tenantId);
    }

    record Greeting(String message, String tenant) {}
...
2. *Validates the identifier*. Checks the syntax of the resolved identifier with a xref:tenant-details.adoc#_tenant_identifier_validation[`TenantIdentifierValidator`]. This always happens, whether or not tenant details are configured.
3. *Verifies the tenant (optional)*. If a xref:tenant-details.adoc#_tenant_verification[`TenantVerifier`] is configured, checks that the tenant exists and is enabled.
4. *Binds the context*. Binds the tenant identifier to a xref:tenant-context.adoc[`ScopedValue`] via `TenantContext`, making it available throughout the request scope.
5. *Publishes a context event*. Fires a `TenantContextAttachedEvent` that triggers registered xref:tenant-context.adoc#_tenant_context_events[listeners] (e.g., MDC enrichment).
6. *Executes the request*. The request proceeds with the tenant context available. All Micrometer observations are automatically enriched with the tenant identifier.
7. *Cleans up*. After the request completes, fires a `TenantContextClosedEvent`. The `ScopedValue` binding is automatically removed when the scope exits.
...
* *xref:tenant-details.adoc[Tenant Details]*: Tenant identifier validation via `TenantIdentifierValidator`, tenant metadata management via `TenantDetailsService`, and verification via `TenantVerifier`.
...
The xref:data-jdbc.adoc[Data JDBC module] (`arconia-multitenancy-data-jdbc`) adds tenant-aware data access for the database-per-tenant isolation strategy:

* A `TenantDataSource` that routes each connection request to the database belonging to the current tenant.
* Per-tenant data sources registered upfront or created on demand and cached.

It builds on plain JDBC and works with `JdbcClient`, `JdbcTemplate`, and Spring Data JDBC alike. Unlike the other modules, it ships no auto-configuration, so it has no starter: add the artifact and declare the `TenantDataSource` bean yourself.
