...
* A new `arconia.dev.services.enabled` configuration property has been introduced to provide a global option for disabling all Dev Services at once. By default, they are all enabled.
...
* When Spring AI PGVector is in the classpath, the PostgreSQL Dev Service will use a `docker.io/pgvector/pgvector` container image as the default. Otherwise, it will use the `postgres` container image.
